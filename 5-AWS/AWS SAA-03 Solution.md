...
**Note**
**A.** 
- S3 Tranfer Acceleration: Uses AWS global edge locations (CloudFront Network)
- Data is uploaded to the nearest edge location, then carried over AWS's optimized backbone network to the destination bucket's Region 
So instead of:
```
Site → Internet → S3 bucket (far away)
```
It becomes:
```
Site → Nearest AWS Edge → AWS backbone → S3 bucket
```
Which is Lower latency and Higher throughput for cross-continent uploads

**B.**
CRR-Cross Region Replication Process
```
Site-Local S3 bucket-CRR-Destination S3 bucket 
```
...
- No/low bandwidth
- PB-scale migrations 
- One-time or rare bulk transfer
but there they have high-speed internet, data is daily(500GB/day per site)
---
